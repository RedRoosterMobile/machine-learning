...
for i = 1: numFilters
    for j = 1: numImages
         ...
         convolvedImage =  squeeze(activations(:, :, i, j));
...
for filterNum = 1: numFilters
     for imageNum = 1: numImages
         %filterNum
         %imageNum
         delta_p_f = squeeze(delta_p_2(:,:,filterNum,imageNum));
         delta_c_f = (1/poolDim^2) * kron( delta_p_f, ones(poolDim) );
         activation_f = squeeze(activations(:, :, filterNum, imageNum));
         delta_c(:,:,filterNum,imageNum) = delta_c_f .* activation_f .* (1-activation_f);
     end
end
%%======================================================================
...
for filterNum = 1:numFilters
    for imageNum = 1: numImages
        ...
        filter = rot90(squeeze(filter),2);
        im = squeeze(images(:, :, imageNum));
        Wc_grad(:,:,filterNum)= squeeze(Wc_grad(:,:,filterNum))+conv2( im, filter, 'valid');
        delta_c_b = squeeze(delta_c(:,:,filterNum,imageNum));
        bc_grad(filterNum) = bc_grad(filterNum) + sum(sum(delta_c_b));
    end
end  
