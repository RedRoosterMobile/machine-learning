...
% We standardize the data so that each pixel will have roughly zero mean
% and unit variance.
s=std(images,[],2);
m=mean(images,2);
images=bsxfun(@minus, images, m);
%images=bsxfun(@rdivide, images, s+.1);
...
if DEBUG
    ...
    % Use this to visually compare the gradients side by side: error check
    % and detect
    %disp([numGrad grad]); 
...
s_test=std(testImages,[],2);
m_test=mean(testImages,2);
testImages=bsxfun(@minus, testImages, m_test);
%testImages=bsxfun(@rdivide, testImages, s_test+.1);

testImages = reshape(testImages,imageDim,imageDim,[]);
testLabels = loadMNISTLabels('../common/t10k-labels-idx1-ubyte');
testLabels(testLabels==0) = 10; % Remap 0 to 10
%testLabels = testLabes + 1;
